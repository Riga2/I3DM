...
def generate_list_from_indices(metadata_dir, indices_path, output_dir, output_name):
    ...
    logging.info(f"Generated {output_path} with {len(json_files)} entries from {indices_path}")
    if missing_files:
        logging.warning(f"Skipped {len(missing_files)} missing metadata files from {indices_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_indices_path", type=str, default='configs/re10k_test_indices.txt')
    parser.add_argument("--test_list_name", type=str, default='re10k_test_list.txt')
    ...
    if cur_mode == "test" and args.test_indices_path:
        generate_list_from_indices(
            search_list_dir,
