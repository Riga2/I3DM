...
def export_results(
    ...
    for batch_idx in range(input_data.image.size(0)):
        ...
        if hasattr(result, "video_rendering"):
            ...
            _save_video(result.video_masks[batch_idx], sample_dir, video_name='rendered_mask.mp4')
...
def visualize_intermediate_results(out_dir, result):
    ...
    if result.render is not None:
        ...
        if result.conf is not None:
            conf = result.conf.reshape(b * v, -1, h, w).expand_as(rendered_image)
        if result.gt_conf is not None:
            gt_conf = result.gt_conf.reshape(b * v, -1, h, w).expand_as(rendered_image)
        # min_vals = conf.amin(dim=[2, 3, 4], keepdim=True)
        # max_vals = conf.amax(dim=[2, 3, 4], keepdim=True)
        # conf_normalized = (conf - min_vals) / (max_vals - min_vals)
        # conf_normalized = conf_normalized.reshape(b * v, -1, h, w).expand_as(rendered_image)
        if result.conf is not None:
            visualized_image = torch.cat((target_image, rendered_image, conf, gt_conf), dim=3).detach().cpu()
        elif getattr(result, 'confidence_score', None) is not None:
             conf_score = result.confidence_score
             
             # Save average confidence score
             ...
             if conf_score.dim() == 5: # [b, v, 1, h, w]
                 avg_conf = conf_score.mean(dim=(2, 3, 4)) # [b, v]
             else:
                 avg_conf = conf_score.reshape(b, v, -1).mean(dim=-1)

             with open(os.path.join(out_dir, f"confidence_avg_{uid_based_filename}.txt"), "w") as f:
                 for b_idx in range(avg_conf.shape[0]):
                     f.write(f"Batch {b_idx} (UID {uids[b_idx]}):\n")
                     for v_idx in range(avg_conf.shape[1]):
                         val = avg_conf[b_idx, v_idx].item()
                         f.write(f"  View {v_idx}: {val:.6f}\n")
                     f.write(f"  Batch Avg: {avg_conf[b_idx].mean().item():.6f}\n")

             if conf_score.dim() == 5:
                 conf_score = conf_score.reshape(b * v, 1, h, w)
             
             # Normalize per image to [0, 1]
             c_min = conf_score.amin(dim=(2, 3), keepdim=True)
             c_max = conf_score.amax(dim=(2, 3), keepdim=True)
             conf_norm = (conf_score - c_min) / (c_max - c_min + 1e-8)
             
             # Expand to 3 channels
             conf_img = conf_norm.repeat(1, 3, 1, 1)
             visualized_image = torch.cat((target_image, rendered_image, conf_img), dim=3).detach().cpu()
        elif result.gt_conf is not None:
            target_mask = target.target_mask.reshape(b * v, -1, h, w)
            target_image = ((target_image * 2. - 1.) * target_mask) * 0.5 + 0.5                 # 0.5 is masked region
...
def _save_images(result, batch_idx, out_dir):
    ...
    last_img = (input_img[-1].permute(1,2,0).cpu().numpy() * 255.0).clip(0.0, 255.0).astype(np.uint8)
    Image.fromarray(last_img).save(os.path.join(out_dir, "last_input.png"))
    ...
    comparison = torch.cat(
        (result.target.image[batch_idx], result.target.image[batch_idx] * result.gt_conf[batch_idx], result.render[batch_idx]), 
        dim=2
    ).detach().cpu()
...
def _save_video(frames, out_dir, video_name="rendered_video.mp4"):
    """
    Save video from rendered frames.
    Input frames should be in [v, c, h, w] format.
    """
    frames = np.ascontiguousarray(np.array(frames.to(dtype=torch.float32, device='cpu')))
