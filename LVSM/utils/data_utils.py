...
class ProcessDataNew(nn.Module):
    ...
    def forward(self, data_batch, compute_rays=True):
        ...
        Returns:
            data_batch (dict): Contains processed tensors with the following keys:
                - 'image' (torch.Tensor): Shape [b, v, c, h, w]
                - 'fxfycxcy' (torch.Tensor): Shape [b, v, 4]
                - 'c2w' (torch.Tensor): Shape [b, v, 4, 4]
        ...
        data_batch["image_h_w"] = (height, width)
...
class ProcessDataLatest(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
    ...
    def fetch_views(self, data_batch, has_target_image=False):
        """
        Splits the input data batch into input and target sets.
        ...
        num_target_views, num_views, bs = self.config.training.num_target_views, data_batch["c2w"].size(1), data_batch["c2w"].size(0)
        num_input_views = num_views - num_target_views
        assert num_target_views < num_views, f"We have {num_views} views, but we want to select {num_target_views} target views. This is more than the total number of views we have."

        for key, value in data_batch.items():
            if key == "fxfycxcy" or key == "c2w":
                input_dict[key] = value[:, :num_input_views, ...]
                target_dict[key] = value[:, -num_target_views:, ...]
            elif key == "image":
                input_dict[key] = value[:, :num_input_views, ...]
                if has_target_image:
                    target_dict[key] = value[:, -num_target_views:, ...]
            elif key == "target_mask" and has_target_image:
                target_dict[key] = value
            else:
                input_dict[key] = value
                target_dict[key] = value
    ...
    @torch.no_grad()
    def forward(self, data_batch, has_target_image=False, compute_rays=True):
        """
        Preprocesses the input data batch and (optionally) computes ray_o and ray_d.
        ...
        Returns:
            Input and Target data_batch (dict): Contains processed tensors with the following keys:
                ...
                - 'image_h_w' (tuple): (height, width)
        """
        # assert all(data_batch['target_len'] > 0) == has_target_image, "If target_len > 0, has_target_image must be True."
        input_dict, target_dict = self.fetch_views(data_batch, has_target_image=has_target_image)
...
class ProcessDataTokens(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
    ...
    def fetch_views(self, data_batch, has_target_image=False):
        ...
        # num_target_views, num_views = self.config.training.num_target_views, data_batch["c2w"].size(1)
        # num_input_views = num_views - num_target_views

        num_input_views = self.config.training.num_tokens * self.config.training.num_frames_per_token

        for key, value in data_batch.items():
            if key == "fxfycxcy" or key == "c2w":
                input_dict[key] = value[:, :num_input_views, ...]
                target_dict[key] = value[:, num_input_views:, ...]
            elif key == "image":
                input_dict[key] = value[:, :num_input_views, ...]
                if has_target_image:
                    target_dict[key] = value[:, num_input_views:, ...]
...
class ProcessDataNew2(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
    ...
    def fetch_views(self, data_batch, has_target_image=False, target_has_input=True):
        ...
        if (self.config.training.get("dynamic_input_view_num", False) 
            and (not self.config.inference.get("if_inference", False))):
            self.config.training.num_input_views = np.random.randint(1, self.config.training.num_views + 1)
        ...
        input_dict["img_index"] = torch.arange(self.config.training.num_input_views, dtype=torch.long, device=data_batch["image"].device).unsqueeze(0).repeat(bs, 1)
        target_dict["img_index"] = index
