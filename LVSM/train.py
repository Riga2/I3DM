...
print(f"os.environ['OMP_NUM_THREADS']={os.environ['OMP_NUM_THREADS']}")
...
# Create parameter map for debugging
param_to_name = {p: n for n, p in optimized_param_dict.items()}
...
while cur_train_step <= total_train_steps:
    ...
    if ddp_info.is_main_process:
        ...
        # log in wandb
        if update_grads and ((cur_train_step % config.training.wandb_log_every == 0) or (
