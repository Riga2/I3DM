...
class Dataset(Dataset):
    ...
    def preprocess_frames(self, frames_chosen, image_paths_chosen):
        ...
        for cur_frame, cur_image_path in zip(frames_chosen, image_paths_chosen):
            ...
            if original_image_w != target_w_resize:
                image = image.resize((target_w_resize, target_h_resize), resample=PIL.Image.LANCZOS)
    ...
    def preprocess_input_poses(
        ...
        return in_c2ws, avg_pose
    
    def preprocess_target_poses(
        self,
        avg_pose: torch.Tensor,
        ...
        target_c2ws = avg_pose @ target_c2ws_src 
    ...
    def view_selector_uniform(self, frames):
        ...
        min_frame_dist = view_selector_config.get("min_frame_dist", 32)
        max_frame_dist = min(len(frames) - 1, view_selector_config.get("max_frame_dist", 192))
        ...
        valid_range = list(range(start_frame, end_frame))
        num_targets = self.config.training.num_target_views
        num_inputs = self.config.training.num_input_views

        context_indices = sorted(random.sample(valid_range, k=num_inputs))
        
        valid_range_len = len(valid_range)
        if num_targets <= 1:
            target_indices = [valid_range[valid_range_len // 2]]
        else:
            positions = [int(i * (valid_range_len - 1) / (num_targets - 1)) for i in range(num_targets)]
            target_indices = sorted([valid_range[pos] for pos in positions])
    ...
    def view_selector(self, frames):
        ...
        context_indices = random.sample(valid_range, k=num_inputs)
        target_indices = random.sample(valid_range, k=num_targets)
    ...
    def __getitem__(self, idx):
        while True:
            try:
                ...
                context_indices, target_indices = self.view_selector(frames)
                ...
                # ctx_c2ws, ctx_avg_pose = self.preprocess_input_poses(input_c2ws[:len(context_indices)], scene_scale_factor)
                # target_c2ws = self.preprocess_target_poses(ctx_avg_pose, input_c2ws[len(context_indices):], scene_scale_factor)
