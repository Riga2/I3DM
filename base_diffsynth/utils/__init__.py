...
class BasePipeline(torch.nn.Module):
    ...
    def preprocess_video_tensor(self, input_video, torch_dtype=None, device=None,):
        video = input_video[None].permute(0,2,1,3,4).to(dtype=torch_dtype or self.torch_dtype, device=device or self.device) # (B C T H W)
        video = video * 2.0 - 1.0
        return video
    ...
    def freeze_except(self, model_names):
        for name, model in self.named_children():
            ...
            #         if "image_token_decoder" in param_name:
        ...
        print(f"Trainable_params (except LoRA): {trainable_params / 1e6:.2f}M")                
    ...
    def freeze_except_new(self, module_name_substrings):
        ...
        for name, module in self.named_modules():
            ...
            if should_be_kept_trainable:
                module.train()
                print(f"✅ Module '{name}' and its parameters are set to TRAINABLE.")
            else:
                module.eval()
    ...
    def add_extra_trainable_module(self, module_name_substrings):
        ...
        print(f"All trainable_params: {trainable_params / 1e6:.2f}M ({trainable_params / total_params * 100:.2f}%)")
