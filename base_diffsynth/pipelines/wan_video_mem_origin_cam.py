...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None):
        ...
        self.units = [
            ...
            # WanVideoUnit_ImageEmbedderCLIP_Single(),
            # WanVideoUnit_ImageEmbedderFused(),
            # WanVideoUnit_FunControl(),
            # WanVideoUnit_FunReference(),
            # WanVideoUnit_FunReferenceNew(),
            WanVideoUnit_FunCameraControl_Plucker(),
            # WanVideoUnit_FunCameraControl_Plucker_Video(),
...
class WanVideoUnit_SceneNVSModel(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, input_nvs_image, input_nvs_c2w, input_nvs_fxfycxcy, target_nvs_c2w, target_nvs_fxfycxcy, target_nvs_image=None, debug_save_dir=None):
        ...
        reference_latents = nvs_latents[:,:,0]                      # B*T,C,H,W
        reference_latents = pipe.ref_conv(reference_latents)
        reference_latents = rearrange(reference_latents, "(b t) c h w -> b (t h w) c", b=1).contiguous()

        if pipe.image_encoder is None:
            return {"reference_latents": reference_latents}
        
        clip_feature = rearrange(pipe.preprocess_video_tensor(input_nvs_image), "b c t h w -> (b t) c h w")     # T,C,H,W
        clip_feature = pipe.image_encoder.encode_image([clip_feature]).mean(dim=0, keepdim=True)   # 1,C,D
...
class WanVideoUnit_ImageEmbedderCLIP_Single(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("input_image", "end_image", "height", "width"),
            onload_model_names=("image_encoder",)
    ...
    def process(self, pipe: WanVideoPipeline, input_image, end_image, height, width):
        ...
        pipe.load_models_to_device(self.onload_model_names)
        # image = pipe.preprocess_image(input_image.resize((width, height))).to(pipe.device)
        image = pipe.preprocess_video_tensor(input_image[:1]).squeeze(2)     # B,C,H,W
        clip_context = pipe.image_encoder.encode_image([image])
        if end_image is not None:
            end_image = pipe.preprocess_video_tensor(end_image[None]).squeeze(2)     # B,C,H,W
            if pipe.dit.has_image_pos_emb:
...
class WanVideoUnit_FunCameraControl_Plucker_Video(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("num_frames", "cam_c2w", "cam_intric", "height", "width", "latents", "input_image", "tiled", "tile_size", "tile_stride"),
            onload_model_names=("vae",)
    ...
    def process(self, pipe: WanVideoPipeline, num_frames, cam_c2w, cam_intric, height, width, latents, input_image, tiled, tile_size, tile_stride):
        ...
        input_latents = pipe.preprocess_video_tensor(input_image)
        input_latents = pipe.vae.encode(input_latents, device=pipe.device)
        # y = torch.zeros_like(latents).to(pipe.device)
        # y[:, :, :1] = input_latents
        y = input_latents.to(dtype=pipe.torch_dtype, device=pipe.device)
