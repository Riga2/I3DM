...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None):
        ...
        self.units = [
            ...
            WanVideoUnit_MemoryEmbedderVAE(),
    ...
    def __call__(
        ...
        # Extra input
        mem_frames: torch.Tensor = None,
        mem_c2w: torch.Tensor = None,
        mem_intric: torch.Tensor = None,
        num_global: torch.Tensor = None,
        num_local: torch.Tensor = None,

        target_c2w: torch.Tensor = None,
        target_intric: torch.Tensor = None,
        ...
        inputs_shared = {
            ...
            "mem_c2w": mem_c2w,
            "mem_intric": mem_intric,
            "num_global": num_global,
            "num_local": num_local,
            "target_c2w": target_c2w,
            "target_intric": target_intric,
...
class WanVideoUnit_NoiseInitializer_Uncompressed(PipelineUnit):
    def __init__(self):
        super().__init__(input_params=("height", "width", "num_frames", "seed", "rand_device", "vace_reference_image"))
...
class WanVideoUnit_MemoryEmbedderVAE(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("mem_frames", "mem_c2w", "mem_intric", "target_c2w", "target_intric", "num_global", "num_local", "height", "width", "tiled", "tile_size", "tile_stride"),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideoPipeline, mem_frames, mem_c2w, mem_intric, target_c2w, target_intric, num_global, num_local, height, width, tiled, tile_size, tile_stride):
        if mem_frames is None or not pipe.dit.require_vae_embedding:
            return {}
        pipe.load_models_to_device(self.onload_model_names)
        mem_video = pipe.preprocess_video_tensor(mem_frames) # B,C,T,H,W
        y = pipe.vae.encode(mem_video, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride) # b,c,t,h,w
        y = y.to(dtype=pipe.torch_dtype, device=pipe.device)

        camera_control_plucker_embedding = pipe.dit.control_adapter.process_camera_params(target_c2w, target_intric, height, width)
        
        control_camera_video = camera_control_plucker_embedding.permute([3, 0, 1, 2]).unsqueeze(0)
        ...
        return {"y": y, "control_camera_latents_input": control_camera_latents_input}

        # all_cam_c2w = torch.cat([target_c2w, mem_c2w])
        # all_cam_intric = torch.cat([target_intric, mem_intric])
        # mem_len = num_global+num_local
        # camera_control_plucker_embedding = pipe.dit.control_adapter.process_camera_params(all_cam_c2w, all_cam_intric, height, width, ref_k=mem_len)

        # # x, y -> latent, cond
        # control_camera_latents_input = torch.cat([target_control_camera_latents, mem_control_camera_latents], dim=1)

        # return {"y": y, "control_camera_latents_input": control_camera_latents_input}
    
        # memory camera emb
        # mem_control_camera_video = camera_control_plucker_embedding[:mem_len].permute([3, 0, 1, 2]).unsqueeze(0)
        # mem_control_camera_latents = torch.concat(
        #     [
        #         torch.repeat_interleave(mem_control_camera_video[:, :, 0:1], repeats=4, dim=2),
        #         mem_control_camera_video[:, :, 1:]
        #     ], dim=2
        # ).transpose(1, 2)
        # b, f, c, h, w = mem_control_camera_latents.shape
        # mem_control_camera_latents = mem_control_camera_latents.contiguous().view(b, f // 4, 4, c, h, w).transpose(2, 3)
        # mem_control_camera_latents = mem_control_camera_latents.contiguous().view(b, f // 4, c * 4, h, w).transpose(1, 2)
        # mem_control_camera_latents = mem_control_camera_latents.to(device=pipe.device, dtype=pipe.torch_dtype)

        # # target camera emb
        # target_control_camera_video = camera_control_plucker_embedding[mem_len:].permute([3, 0, 1, 2]).unsqueeze(0)
        # target_control_camera_latents = torch.concat(
        #     [
        #         torch.repeat_interleave(target_control_camera_video[:, :, 0:1], repeats=4, dim=2),
        #         target_control_camera_video[:, :, 1:]
        #     ], dim=2
        # ).transpose(1, 2)
        # b, f, c, h, w = target_control_camera_latents.shape
        # target_control_camera_latents = target_control_camera_latents.contiguous().view(b, f // 4, 4, c, h, w).transpose(2, 3)
        # target_control_camera_latents = target_control_camera_latents.contiguous().view(b, f // 4, c * 4, h, w).transpose(1, 2)
        # target_control_camera_latents = target_control_camera_latents.to(device=pipe.device, dtype=pipe.torch_dtype)
        # return {"y": y, "mem_cam_latents": mem_control_camera_latents, "target_cam_latents": target_control_camera_latents}
...
def model_fn_wan_video(
    ...
    mem_cam_latents = None,
    target_cam_latents = None,
    ...
    **kwargs,
    ...
    # x = dit.patchify_new(x, mem_cam_latents, target_cam_latents)
