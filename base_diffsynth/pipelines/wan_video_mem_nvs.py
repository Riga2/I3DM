...
from ..models.wan_scene_nvs_model import SceneNVSModel
...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None):
        ...
        self.nvs_model = SceneNVSModel().to(device=device, dtype=torch_dtype)
        ...
        self.units = [
            ...
            WanVideoUnit_NoiseInitializerNew(),
            WanVideoUnit_PromptEmbedder(),
            WanVideoUnit_InputVideoEmbedderNew(),
    ...
    def __call__(
        ...
        inputs_shared["latents"] = rearrange(inputs_shared["latents"], "b c f h w -> (b f) c h w").unsqueeze(2)
        video = self.vae.decode(inputs_shared["latents"], device=self.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)
        video = rearrange(video, "(b f) c 1 h w -> b c f h w", b=1)
...
class WanVideoUnit_NoiseInitializerNew(PipelineUnit):
    def __init__(self):
        super().__init__(input_params=("height", "width", "num_frames", "seed", "rand_device", "vace_reference_image"))

    def process(self, pipe: WanVideoPipeline, height, width, num_frames, seed, rand_device, vace_reference_image):
        shape = (1, pipe.vae.model.z_dim, num_frames, height // pipe.vae.upsampling_factor, width // pipe.vae.upsampling_factor)
        noise = pipe.generate_noise(shape, seed=seed, rand_device=rand_device)
        return {"noise": noise}
...
class WanVideoUnit_InputVideoEmbedderNew(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("input_video", "noise", "tiled", "tile_size", "tile_stride", "vace_reference_image"),
            onload_model_names=("vae",)
    ...
    def process(self, pipe: WanVideoPipeline, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image):
        ...
        input_video = rearrange(pipe.preprocess_video_tensor(input_video), "b c t h w -> (b t) c h w").unsqueeze(2).to(dtype=pipe.torch_dtype, device=pipe.device)   # B*T,C,1,H,W
        input_latents = pipe.vae.encode(input_video, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device)
        input_latents = rearrange(input_latents, "(b t) c v h w -> b c (t v) h w", b=1, v=1)
...
class WanVideoUnit_SceneNVSModel(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, input_nvs_image, input_nvs_c2w, input_nvs_fxfycxcy, target_nvs_c2w, target_nvs_fxfycxcy, target_nvs_image=None, debug_save_dir=None):
        ...
        input_imgs = rearrange(pipe.preprocess_video_tensor(input_nvs_image), "b c t h w -> (b t) c h w").unsqueeze(2).to(dtype=pipe.torch_dtype, device=pipe.device)   # B*T,C,1,H,W
        input_latents = pipe.vae.encode(input_imgs, device=pipe.device).to(dtype=pipe.torch_dtype, device=pipe.device)
        input_latents = rearrange(input_latents, "(b t) c v h w -> b (t v) c h w", b=1, v=1)
        ...
        with autocast(device_type="cuda", dtype=torch.bfloat16):
            nvs_latents = pipe.nvs_model(data_input_batch, data_target_batch) # b c v h w
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    if y is not None:
        _, c_y, f_y, h_y, w_y = y.shape
        y = rearrange(y, 'b c f h w -> b (f h w) c').contiguous()
