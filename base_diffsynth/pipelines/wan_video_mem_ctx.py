...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16, tokenizer_path=None):
        ...
        self.units = [
            ...
            WanVideoUnit_InputVideoEmbedderCTX(),
    ...
    def __call__(
        ...
        input_ctx_image: torch.Tensor = None,
        ...
        inputs_shared = {
            ...
            "input_ctx_image": input_ctx_image,
...
class WanVideoUnit_InputVideoEmbedderCTX(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("input_ctx_image", "tiled", "tile_size", "tile_stride"),
            onload_model_names=("vae","ref_conv")
        )

    def process(self, pipe: WanVideoPipeline, input_ctx_image, tiled, tile_size, tile_stride):
        if input_ctx_image is None:
            return None
        pipe.load_models_to_device(["vae","ref_conv"])
        input_ctx_image = rearrange(pipe.preprocess_video_tensor(input_ctx_image), "b c t h w -> (b t) c h w").unsqueeze(2).to(dtype=pipe.torch_dtype, device=pipe.device)   # B*T,C,1,H,W
        input_ctx_latents = pipe.vae.encode(input_ctx_image, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device) # B*T,C,1,H,W
        
        ctx_latents = input_ctx_latents[:,:,0]                      # B*T,C,H,W
        ctx_latents = pipe.ref_conv(ctx_latents)
        ctx_latents = rearrange(ctx_latents, "(b t) c h w -> b (t h w) c", b=1).contiguous()        
        
        return {"reference_latents": ctx_latents}
...
class WanVideoUnit_FunCameraControl_Plucker(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, num_frames, cam_c2w, cam_intric, height, width, latents, input_image, tiled, tile_size, tile_stride):
        ...
        ctx_camera_plucker = camera_control_plucker_embedding[num_frames:].permute([3, 0, 1, 2]).unsqueeze(0)
        ctx_camera_latents = torch.repeat_interleave(ctx_camera_plucker, repeats=4, dim=2).transpose(1, 2)
        b, f, c, h, w = ctx_camera_latents.shape
        ctx_camera_latents = ctx_camera_latents.contiguous().view(b, f // 4, 4, c, h, w).transpose(2, 3)
        ctx_camera_latents = ctx_camera_latents.contiguous().view(b, f // 4, c * 4, h, w).transpose(1, 2)
        ctx_camera_latents_input = ctx_camera_latents.to(device=pipe.device, dtype=pipe.torch_dtype)    
        ...
        return {"control_camera_latents_input": control_camera_latents_input, "y": y, "ctx_camera_latents_input": ctx_camera_latents_input}
...
def model_fn_wan_video(
    ...
    ctx_camera_latents_input: Optional[torch.Tensor] = None,
    vace_context = None,
    vace_scale = 1.0,
    audio_embeds: Optional[torch.Tensor] = None,
    ...
    **kwargs,
    ...
    if reference_latents is not None:
        reference_latents = dit.add_cam_emb(reference_latents, ctx_camera_latents_input)
        x = torch.concat([x, reference_latents], dim=1)
        f *=2
