...
class AttentionModule(nn.Module):
    ...
    def forward(self, q, k, v, return_weights=False):
        ...
        if return_weights: 
            ...
            attn = attn.softmax(dim=-1)         # [B, Num_Heads, Seq_Len, Seq_Len]
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, freqs, return_weights=False):
        ...
        if return_weights:
            x, attn_map = self.attn(q, k, v, return_weights=True)
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, context, t_mod, freqs, return_weights=False):
        ...
        if return_weights:
            attn_x, attn_map = self.self_attn(input_x, freqs, return_weights=True)
        else:
            attn_x = self.self_attn(input_x, freqs, return_weights=False)
            attn_map = None
        x = self.gate(x, gate_msa, attn_x)
...
class WanModel(torch.nn.Module):
    ...
    def init_camera_control_adapter(self, in_dim_control_adapter=24, device='cuda', torch_type=torch.bfloat16):
        self.control_adapter = SimpleAdapter(in_dim_control_adapter, self.dim, kernel_size=self.patch_size[1:], stride=self.patch_size[1:]).to(device=device, dtype=torch_type)

    def init_ref_conv(self):
        self.ref_conv = nn.Conv2d(16, self.dim, kernel_size=(2, 2), stride=(2, 2))

    def init_updated_module(self, in_dim=36, device='cuda', torch_type=torch.bfloat16):
        self.in_dim = in_dim
        self.patch_embedding_new = nn.Conv3d(
            in_dim, self.dim, kernel_size=self.patch_size, stride=self.patch_size).to(device=device, dtype=torch_type)
        
    def init_two_cam_adapter(self):
        self.two_cam_adapter = nn.Conv2d(self.dim * 2, self.dim)
    ...
    def add_cam_emb(self, x: torch.Tensor, control_camera_latents_input: Optional[torch.Tensor] = None):
        if self.control_adapter is not None and control_camera_latents_input is not None:
            y_camera = self.control_adapter(control_camera_latents_input)
            y_camera = rearrange(y_camera, 'b c f h w -> b (f h w) c').contiguous()
            x += y_camera
        return x

    def patchify_new(self, x: torch.Tensor, mem_cam_latents: Optional[torch.Tensor] = None, target_cam_latents: Optional[torch.Tensor] = None):
        x = self.patch_embedding(x)
        if self.control_adapter is not None and mem_cam_latents is not None and target_cam_latents is not None:
            y_mem_camera = self.control_adapter(mem_cam_latents)
            y_target_camera = self.control_adapter(target_cam_latents)
            x = [u + v + w for u, v, w in zip(x, y_mem_camera, y_target_camera)]
            x = x[0].unsqueeze(0)
        return x
