...
class SimpleAdapter(nn.Module):
    ...
    def process_camera_params(
        ...
        plucker_embedding = process_pose_file_New(c2w, intrinsc, height, width, ref_k=ref_k)
        return plucker_embedding        
...
class Camera_New(object):
    """Copied from https://github.com/hehao13/CameraCtrl/blob/main/inference.py
    """
    def __init__(self, c2w, intrincs):
        fx, fy, cx, cy = intrincs
        self.fx = fx
        self.fy = fy
        self.cx = cx
        self.cy = cy
...
def get_relative_pose_new(cam_params, ref_k=0):
    """Copied from https://github.com/hehao13/CameraCtrl/blob/main/inference.py
    """
    abs_w2cs = [cam_param.w2c_mat for cam_param in cam_params]
    abs_c2ws = [cam_param.c2w_mat for cam_param in cam_params]
    assert len(abs_w2cs) > ref_k
    ...
    abs2rel = target_cam_c2w @ abs_w2cs[ref_k]

    ret_poses = []
    for i, abs_c2w in enumerate(abs_c2ws):
        relative_pose = abs2rel @ abs_c2w
        ret_poses.append(relative_pose)
...
def process_pose_file_New(c2ws, intrincs, height, width, ref_k=0, device='cuda'):
    cam_params = [Camera_New(c2w.cpu().numpy(), intrinc.cpu().numpy()) for c2w, intrinc in zip(c2ws, intrincs)]

    K = intrincs[None].to(device)  # [1, n_frame, 4]
    c2ws = get_relative_pose_new(cam_params, ref_k=ref_k)  # Assuming this function is defined elsewhere
    c2ws = torch.as_tensor(c2ws)[None].to(device)  # [1, n_frame, 4, 4]
    plucker_embedding = ray_condition(K, c2ws, height, width, device=device)[0].permute(0, 3, 1, 2).contiguous()  # V, 6, H, W
    plucker_embedding = plucker_embedding[None]
    plucker_embedding = rearrange(plucker_embedding, "b f c h w -> b f h w c")[0]
    return plucker_embedding
