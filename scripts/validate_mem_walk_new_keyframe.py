...
class ValidationDataset(torch.utils.data.Dataset):
    def __init__(self, metadata_path):
        ...
        # self.all_scene_paths = [path.strip() for path in lines if path.strip()][:20]
        ...
        base_dir = '/mnt/afse2/DATASET/re10k_preprocessed/test/metadata'
...
base_results_dir = './results/Keyframes_SceneDec_ft_s11000_occSelection'
...
for data in tqdm(test_dataloader):
    ...
    for ind in tqdm(range(len(clips_captions))):
        ...
        video = pipe(
            ...
            prompt=cur_caption,
            negative_prompt="色调艳丽，过曝，细节模糊不清，整体发灰，最差质量，低质量，JPEG压缩残留，丑陋的，残缺的，畸形的，杂乱的背景,不完整的",
            seed=1, tiled=True,
