...
base_results_dir = './results/Keyframes_SceneDec_ft_s11000_occSelect_all_wNeg'
...
for data in tqdm(test_dataloader):
    ...
    for ind in tqdm(range(len(clips_captions))):
        ...
        # retrieved_result, retrieved_ratios = mem_bank.retrieve_top_k_cameras_learned_topK(
        retrieved_result = mem_bank.retrieve_top_k_cameras_learned_use(
            query_c2w=cur_traj[::4],
            query_fxfycxcy=cur_fxfycxcy[::4],
            k=ctx_len,
        )
        # retrieved_ratios = [0.0] * len(retrieved_result)
