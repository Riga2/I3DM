...
import json
from frame_memory_retrieval_geo import KeyFrameMemoryBankGeometryRetrieval
...
class ValidationDataset(torch.utils.data.Dataset):
    def __init__(self, metadata_path, rank=0, world_size=1):
        ...
        all_lines = [path.strip() for path in lines if path.strip()][:200]
...
base_results_dir = './results/Keyframes_SceneDec_ft_s11000_geometrySelect_all_wNeg'
...
num_per_clip = 77
mem_bank = KeyFrameMemoryBankGeometryRetrieval(n_sample_points=100000, use_surfel=True)
...
for data in tqdm(test_dataloader):
    ...
    for ind in tqdm(range(len(clips_captions))):
        ...
        retrieved_result = mem_bank.retrieve_top_k_surfel(
            query_c2w=cur_traj[::4],
            query_fxfycxcy=cur_fxfycxcy[::4],
            k=ctx_len,
            debug_dir=save_results_dir,
        )
    ...
    # Only save logs and video if main process
    # But wait, every process runs different scenes, so every process should save its own results.
    # The original code saves per scene, so it's fine.
